...
const RecruitmentOverseasSection = () => {
    ...
    <section className="relative font-manrope max-w-7xl mx-auto lg:px-6 md:px-12 px-4 md:py-16 py-10">
      <div className="flex md:flex-row flex-col gap-8 lg:justify-center mb-12">
        <h2 className="max-w-xl text-center xl:text-5xl md:text-4xl text-3xl font-extrabold my-4">
          <span className="text-[#0E313A]">Recruitment & </span>
          <span className="text-[#C40808]">Overseas </span>
          <span className="text-[#0E313A]">Employment </span>
        </h2>
      </div>

      <div className="hover:text-white grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 xl:gap-10 lg:gap-6 gap-10">
  ...
  >
    ...
    <div className="absolute xl:bottom-12 lg:bottom-6 md:bottom-10 bottom-10 xl:left-10 lg:left-6 left-8 z-0 pointer-events-none">
      <span className="block w-8 h-8 rounded-full bg-[#0E313A] transition-transform duration-700 ease-in-out group-hover:scale-[50] group-hover:bg-[#C40808]"></span>
    </div>
    ...
    <div className="relative z-10 flex flex-col justify-between h-full xl:py-12 lg:p-6 py-10 xl:px-10 px-8 pointer-events-none">
      ...
      <div className="flex items-center gap-4">
        ...
        />
        <h3 className="text-[22px] font-extrabold text-[#0E313A] transition-colors duration-500 group-hover:text-white">
          {job.title}
        </h3>
      </div>

      <p className="text-[16px] font-medium text-[#666666] transition-colors duration-500 group-hover:text-gray-200">
        {job.subtitle}
      </p>

      <div className="flex items-center gap-2 pt-3">
        {/* Visible Arrow Icon (Transparent background because expansion is behind it) */}
        <div className="w-8 h-8 flex items-center justify-center rounded-full transition-all duration-500 group-hover:bg-[#0E313A]">
          <MdArrowOutward className="text-white text-xl transition-transform duration-500 group-hover:rotate-45" />
        </div>
        
        <span className="text-[15px] text-[#0E313A] font-extrabold transition-colors duration-500 group-hover:text-white ml-2">
          Know More
        </span>
      </div>
    </div>
